...
class AoTProcessor:
    ...
    def run(self, problem_text: str) -> Tuple[AoTResult, str]:
        ...
        original_configured_max_steps = self.runner_config.max_steps # MODIFIED
        ...
        logging.info(f"Main Models: {', '.join(self.runner_config.main_model_names)}, LLMConfig: {self.llm_config}") # MODIFIED
        if self.runner_config.max_reasoning_tokens: # MODIFIED
            logging.info(f"Reasoning Token Limit: {self.runner_config.max_reasoning_tokens}") # MODIFIED
        logging.info(f"Max Time Limit (Overall Process): {self.runner_config.max_time_seconds}s") # MODIFIED
        logging.info(f"Original Max Steps Config: {original_configured_max_steps}")
        if self.runner_config.pass_remaining_steps_pct is not None: # MODIFIED
             trigger_step_for_advisory = self.runner_config.pass_remaining_steps_pct * original_configured_max_steps # MODIFIED
             logging.info(f"Remaining steps advisory will trigger at/after step {trigger_step_for_advisory:.1f} (of original max steps)")
        logging.info(f"No Progress Limit: {self.runner_config.no_progress_limit} steps") # MODIFIED
        ...
        if not result.final_answer:
            ...
            if can_make_final_call:
                ...
                final_reply_content, final_stats = self.llm_client.call(
                    final_prompt, models=self.runner_config.main_model_names, config=self.llm_config # MODIFIED
                )
